...
describe('moveStop', () => {
  test('moves freely within [0, distanceKm]', () => {
    const stop: Stop = { id: 1, at: 50, name: 'Stop' };
    expect(moveStop(stop, 100, 10)).toBe(60);
  });

  test('clamps at the route start', () => {
    const stop: Stop = { id: 1, at: 10, name: 'Stop' };
    expect(moveStop(stop, 100, -50)).toBe(0);
  });

  test('clamps at the route end', () => {
    const stop: Stop = { id: 1, at: 90, name: 'Stop' };
    expect(moveStop(stop, 100, 50)).toBe(100);
  });
});
...
describe('clampStopToDistance', () => {
  test('leaves a stop stop alone when it already fits', () => {
    const stop: Stop = { id: 1, at: 40, name: 'Stop' };
    expect(clampStopToDistance(stop, 100)).toBe(stop);
  });

  test('pulls it back to the new end of the route', () => {
    const stop: Stop = { id: 1, at: 90, name: 'Stop' };
    expect(clampStopToDistance(stop, 50)).toEqual({ ...stop, at: 50 });
  });
});

describe('nextStopAt', () => {
  test('midpoint between the start and the end when there are no markers yet', () => {
    expect(nextStopAt([], 100)).toBe(50);
  });

  test('midpoint between the last marker and the end', () => {
    const stops: Stop[] = [
      { id: 1, at: 20, name: 'Stop' },
      { id: 2, at: 60, name: 'Stop' },
    ];
    expect(nextStopAt(stops, 100)).toBe(80); // (60 + 100) / 2
  });
});
