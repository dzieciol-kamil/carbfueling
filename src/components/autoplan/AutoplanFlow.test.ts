...
import type { Fill, FoodItem, RouteInput, Stop } from '../../domain/types';
 ...
 * part the rider is likeliest to have kept, because a stop is a real stop he found on the map. A
...
const autoStop: Stop = { id: 1, at: 40, name: 'Sklep', autoCreated: true };
const ownStop: Stop = { id: 2, at: 60, name: 'Żabka' };

describe('needsReplaceConfirm', () => {
  test('a fresh plan goes straight through', () => {
    expect(needsReplaceConfirm({ fills: [], foods: [], stops: [] })).toBe(false);
  });

  test('fills alone are enough to ask', () => {
    expect(needsReplaceConfirm({ fills: [fill], foods: [], stops: [] })).toBe(true);
  });

  test('stops autoplan made last time count, even with the plan emptied by hand', () => {
    expect(needsReplaceConfirm({ fills: [], foods: [], stops: [autoStop] })).toBe(true);
  });

  test("the rider's own stops are never touched, so they are no reason to ask", () => {
    expect(needsReplaceConfirm({ fills: [], foods: [], stops: [ownStop] })).toBe(false);
  });

  test('food alone is enough to ask', () => {
    expect(needsReplaceConfirm({ fills: [], foods: [food], stops: [] })).toBe(true);
  });
});
